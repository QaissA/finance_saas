...
const ImportCard = ({
    ...
    const handleContinue = () => {
        ...
        const formattedData = arrayOfData.map((item) => ({
            ...
            data: format(parse(item.data, dateFormat, new Date()), outputFormat)
        }));
        ...
        <div className='max-w-screen-2xl mx-auto w-full pb-10 -mt-24'>
            <Card className="border-none drop-shadow-sm">
                <CardHeader className='gap-y-2 lg:flex-row lg:items-center lg:justify-between'>
                    <CardTitle className='texm-xl line-clamp-1'>
                        Import Transaction.
                    </CardTitle>
                    <div className='flex flex-col lg:flex-row gap-y-2 items-center gap-x-2'>
                        <Button onClick={onCancel} size="sm" className='w-full lg:w-auto'>Cancel</Button>
                        <Button onClick={handleContinue} disabled={progress < requiredOptions.length} size="sm" className='w-full lg:w-auto'>Continue ({progress} / {requiredOptions.length})</Button>
                    </div>
                </CardHeader>
                <CardContent>
